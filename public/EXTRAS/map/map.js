...
function createMap() {
  ...
  var path = d3.geo
    .path() // path generator that will converts GeoJSON to SVG paths
    ...
    d3.csv("graduates.csv", function(grad) {
      d3.csv("us_zip_codes_SMALL.csv", function(zips) {
        ...
        grad.forEach(function(val, i) {
          const result = zip_obj[val.ZipCode];

          //if it could find a matching zipcode:
          if (result) {
            const { ZipCode, Latitude, Longitude } = result;
            //set "city" for Washington University manually:
            if (ZipCode == "63110") {
              var info = washU.geometry.coordinates;
            } else {
              var hometown = turf.point([+Longitude, +Latitude]);
              ...
              if (turf.distance(hometown, large_result) > 100) {
                ...
                var info = large_result.geometry.coordinates;
              }
            }
        ...
        svg
          .selectAll("circle")
          .data(groupedGrads.sort((a, b) => b.values.length - a.values.length))
          .enter()
          .append("circle")
          ...
          .attr("title", createTooltip)
          .attr("r", function(d) {
            return radius(count.filter((s) => s.key == d.key)[0].values);
          });
        ...
        var color = d3.scale
          .ordinal()
          .range(
            Object.keys(tooltipColors).map((v) => tooltipColors[v].color)
          );
        ...
        legend2
          ...
          .text((d, i) => tooltipColors[Object.keys(tooltipColors)[i]].title);
...
function createTooltip(info) {
  ...
  info.values.sort(function(a, b) {
    return (
      a["Final Year in Residency Program"] -
      b["Final Year in Residency Program"]
    );
  });
  ...
  $.each(info.values, function(i, val) {
    ...
    var prof =
      val[
        "Academic Rank: 0 if not ranked; 1=Fellow; 2=Instructor; 3=Asst Prof; 4=Assoc Prof; 5=Professor"
      ];
    if (isNaN(prof) || prof == "") {
      prof = 0;
    } else if (prof > 0) {
    ...
    content +=
      ...
      val["Last Name"] +
      " (" +
      val["Final Year in Residency Program"] +
      ")</div>";
  });
  return content;
}
